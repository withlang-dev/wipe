...
comptime fn game_target(name: str, entry: str) -> Target:
    target_new(.Executable, name, entry).dep("audio").dep("shaders").input("src/game.w").input("src/input.w").input("src/presentation.w").input("src/audio.w").input("src/metrics.w").input("src/tuning.w").input("src/shaders.w").input("src/paths.w")
